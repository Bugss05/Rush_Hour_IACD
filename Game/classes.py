...
cores = {
        ".": (255, 255, 255),  # Branco (espaço vazio)
        #carros
        2: (255, 0, 0),  # Vermelho (carro do jogador)
        3: (255, 102, 102),  # Rosa (carro)
        4: (3, 255, 28),  # Verde vibrante (carro)
        5: (255, 165, 0),  # Laranja (carro)
        6: (64, 64, 64),  # Cinza escuro (carro)
        7: (0, 100, 0),  # Verde escuro (carro)
        8: (74, 0, 74),  # Rosa Escuro (carro)
        9: (255, 192, 156),  # Beje (carro)
        10: (102, 255, 102),  # Verde claro (carro)
        11: (181, 181, 181),  # Cinza Claro (carro)
        12: (102, 51, 0),  # Castanho (carro)
        13: (34, 139, 34),  # Verde floresta (carro)
        #camioes
        14: (0, 0, 255),  # Azul escuro (caminhão)
        15: (153, 51, 255),  # Lilás (caminhão)
        16: (255, 255, 0),  # Amarelo (caminhão)
        17: (153, 204, 255),  # Azul claro (caminhão)
    }
...
class Botao():
    ...
    def draw(self, tela):
        ...
        if self.rect.collidepoint(posicao_rato):
            if pygame.mouse.get_pressed()[0] == 1 and not self.click:  # Verifica se o botão foi clicado
                self.click = True
                acao = True
        else:
            self.click = False  # Reseta o estado do clique se o rato não estiver sobre o botão
